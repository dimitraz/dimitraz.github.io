...
function frame(time) {
        ...
        speed:
          cursorTrail.length <= 1
            ...
            : ((pos, lastPos) => {
                ...
                return Math.sqrt(
                  Math.pow(lastPos.x - pos.x, 2) +
                    Math.pow(lastPos.y - pos.y, 2)
                )
  ...
  cursor.style.boxShadow = cursorTrail
    .map((pos, i) => {
      ...
      return `${offsetX}px ${offsetY}px ${pos.speed + 1}px ${
        settings.diameter
      }px ${color}`
    })
    .reverse()
